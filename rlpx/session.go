...
func (s *Session) GetInfo() devp2p.Info {
	info := devp2p.Info{
...
func (s *Session) Streams() []devp2p.Stream {
	b := make([]devp2p.Stream, len(s.streams))
...
func (s *Session) OpenStream(offset uint, length uint, spec devp2p.ProtocolSpec) *Stream {
