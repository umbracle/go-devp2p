...
func (p *Peer) Enode() string {
	id := strings.Replace(EncodeToHex(p.Bytes), "0x", "", -1)
...
func (r *rpcNode) toPeer() (*Peer, error) {
	return newPeer(EncodeToHex(r.ID), &net.UDPAddr{IP: r.IP, Port: int(r.UDP)}, r.TCP)
}
...
func NewBackend(logger *log.Logger, key *ecdsa.PrivateKey, transport Transport) (*Backend, error) {
	...
	pub := &key.PublicKey
	id := EncodeToHex(elliptic.Marshal(pub.Curve, pub.X, pub.Y)[1:])
			...
			if b.packetCh != nil {
				...
				go func() {
					if err := b.HandlePacket(packet); err != nil {
						b.logger.Printf("[TRACE] failed to handle packet: err, %v", err.Error())
					}
				}()
			}
		case <-b.shutdownCh:
...
func (b *Backend) loadBootnodes() {
	...
	b.logger.Printf("[INFO] Finished probing bootnodes")
	b.Lookup()
}
...
func decodePeerFromPacket(packet *Packet) (*Peer, error) {
	...
	peer, err := newPeer(EncodeToHex(pubkey[1:]), addr, 0)
	if err != nil {
		return nil, err
	}
...
func (b *Backend) HandlePacket(packet *Packet) error {
	...
	case pongPacket:
		return fmt.Errorf("pongPacket not expected %20s %20s %s", peer.addr(), peer.ID, EncodeToHex(peer.Bytes))
	default:
